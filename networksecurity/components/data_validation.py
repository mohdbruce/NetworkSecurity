...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: pd.DataFrame) -> bool:
        try:
            ...
            logging.info(f"Expected number of columns: {number_of_columns}, Found columns: {len(dataframe.columns)}")
        ...
        try:
            ...
            for column in base_df.columns:
                ...
                if threshold <= is_sample_dist.pvalue:
                    ...
                    is_found = True
                    status = False
            drift_report.update({column: {"pvalue": float(is_sample_dist.pvalue), "drift_status": is_found}})
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not train_status:
                logging.info("Train data does not have the expected number of columns")
            logging.info("Validating number of columns in test data")
            test_status = self.validate_number_of_columns(test_df)
            if not test_status:
                logging.info("Test data does not have the expected number of columns")
            validation_status = train_status and test_status
            logging.info(f"Data validation completed. Validation status: {validation_status}")
